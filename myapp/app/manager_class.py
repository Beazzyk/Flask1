...
class Manager:
    ...
    def zarejestruj_zakup(self, produkt, cena, ilosc):
    ...
    def zarejestruj_sprzedaz(self, produkt, cena, ilosc):
        try:
            if produkt in self.magazyn and self.magazyn[produkt]["stan magazynu(sztuk)"] >= ilosc:
                ...
                print("Nie można zrealizować sprzedaży: produkt niedostępny w wymaganej ilości.")
        except ValueError:
    ...
    def modyfikuj_saldo_dodaj(self, kwota):
        self.saldo += kwota
        print("Saldo zostało zaktualizowane i wynosi", self.saldo)


    def modyfikuj_saldo_odejmij(self, kwota):
        if kwota <= self.saldo:
            self.saldo -= kwota
            print("Saldo zostało zaktualizowane i wynosi", self.saldo)
        else:
